...
class DrivewayDetection( object ):
    ...
    @staticmethod
    def performDrivewayDetection( yolo_model, img:np.ndarray, yolo_threshold:float, kernel_size:float, out_img_shape=(375, 1242) ) -> tuple:
        img = DrivewayDetection.transformImage(img)
        _, da_seg_out,ll_seg_out = yolo_model( img )
        da_seg, ll_seg = DrivewayDetection.postProcessing( da_seg_out, ll_seg_out, yolo_threshold, kernel_size, out_img_shape )
